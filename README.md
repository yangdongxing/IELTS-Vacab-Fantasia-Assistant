...
- **🎧 macOS 原生高保真 Siri 语音朗读**：
  - 点击段落卡片的 `[🎧 Siri朗读]`，连续点击可按 **1 次 ➔ 3 次 ➔ 6 次 ➔ 10 次 ➔ 15 次** 阶梯设定，停顿 600ms 后自动后台唤起系统原生高品质 Siri 连播；再次点击即可随时停止；
  - 点击下方核心短语列表中的任意英文，**默认直接唤起原生 Siri 单次朗读**；
  - **自动降级保护**：若本地未启动 `server.py`，点击将**完全无感、自动降级**调用浏览器内置 Web Speech API 发音，不影响任何阅读流程！
