...
3. **正文标注与双语对照**：
   ...
   - **段落下无侵入插入双语对照卡片**：自动插入神经翻译卡片，配备 `[🔄]` 重试、`[🔊 朗读英文]`（浏览器内置发音 + CSS 高亮逐词追踪）、`[🎧 Siri朗读]`（连续点击阶梯累加 1-3-6-10-15 次循环播放原生 Siri 语音）及 `[✕]` 关闭；
   - **智谱 AI 核心搭配短语一键掌握**：卡片下方智能列出长难句重点语块，**点击任意英文单词即可一键全选短语、复制并立即自动发音**（默认优先呼叫本地原生高保真 Siri，服务离线时自动无缝降级为浏览器语音）。
...
## 🚀 启动本地辅助服务 (可选 / 推荐开启 Siri 朗读)

> 💡 **提示**：本项目的 **3,600+ 高清助记图已全部托管于全球高速 CDN (Cloudflare Workers)**，开箱即用，**日常使用无需下载庞大本地图库**。

本地服务（`server.py`）提供以下高级辅助功能：
- **🎧 macOS 原生高保真 Siri 语音朗读与实时音词跟踪**：
  - 点击段落卡片的 `[🎧 Siri朗读]`，连续点击可按 **1 次 ➔ 3 次 ➔ 6 次 ➔ 10 次 ➔ 15 次** 阶梯设定，停顿 600ms 后自动后台唤起系统原生高品质 Siri 连播；
  - **智能渐进梯级语速（渐入佳境）**：
    - **1 次**：直接使用系统标准速度（1.0x）；
    - **3 次**：第 1 次慢速（~0.8x，听清每个音节）➔ 第 2 次中速（~0.9x，熟悉连读弱读）➔ 第 3 次标准速度（1.0x，自然原速吸收）；
    - **6 / 10 / 15 次**：前 3 次与上述渐进梯级相同（慢速 ➔ 中速 ➔ 标准速度），第 4 次及之后全部保持标准速度持续磨耳朵；
  - **实时音词跟踪**：正文单词随发音实时高亮（纯净聚焦，无整句蓝底打扰）；多遍循环之间留有 0.8s 自然呼吸停顿并自动复位光标从头开始；再次点击即可随时停止；
  - 点击下方核心短语列表中的任意英文，**默认直接唤起原生 Siri 单次朗读**；
  - **离线保留手动模式**：若本地未启动 `server.py`，点击将自动保留文本至剪贴板并提示 `Opt+Esc`，可像以前一样由您手动快捷键触发系统朗读，完全不影响原有阅读流！
...
python3 server.py
```
> 服务默认运行在 `http://127.0.0.1:8777`。保持后台运行即可享受 macOS 顶级原生发音体验！
